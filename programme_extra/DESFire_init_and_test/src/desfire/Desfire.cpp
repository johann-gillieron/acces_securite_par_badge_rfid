...
LOG_MODULE_REGISTER(nfc_desfire, LOG_LEVEL_ERR);

#include "Desfire.h"
#include "Secrets.h"

Desfire::Desfire() : mi_CmacBuffer(mu8_CmacBuffer_Data, sizeof(mu8_CmacBuffer_Data))
{
    mpi_SessionKey       = NULL;
...
bool Desfire::Authenticate(uint8_t u8_KeyNo, DESFireKey* pi_Key)
{
    ...
    // Request a random of 16 uint8_t, but depending of the key the PICC may also return an 8 uint8_t random
    DESFireStatus e_Status;
    uint8_t u8_RndB_enc[16]; // encrypted random B
    int32_t s32_Read = DataExchange(u8_Command, &i_Params, u8_RndB_enc, 16, &e_Status, MAC_None);
    if (e_Status != ST_MoreFrames || (s32_Read != 8 && s32_Read != 16))
    {
        LOG_ERR("Authentication failed (1)");
        return false;
    }

    int32_t s32_RandomSize = s32_Read;
    ...
    {
        LOG_INF("* RndB_enc:  ");
        LOG_HEXDUMP_INF(u8_RndB_enc,  (uint16_t)s32_RandomSize, LF);
        LOG_INF("* RndB:      ");
        LOG_HEXDUMP_INF(u8_RndB,      (uint16_t)s32_RandomSize, LF);
        LOG_INF("* RndB_rot:  ");
        LOG_HEXDUMP_INF(u8_RndB_rot,  (uint16_t)s32_RandomSize, LF);
        LOG_INF("* RndA:      ");
        LOG_HEXDUMP_INF(u8_RndA,      (uint16_t)s32_RandomSize, LF);
        LOG_INF("* RndAB:     ");
        LOG_HEXDUMP_INF(i_RndAB.GetData(),      (uint16_t)(2*s32_RandomSize), LF);
        LOG_INF("* RndAB_enc: ");
        LOG_HEXDUMP_INF(i_RndAB_enc.GetData(),  (uint16_t)(2*s32_RandomSize), LF);
    }
    ...
    {
        LOG_INF("Authentication failed (2)\r\n");
        return false;
    }
    ...
    {
        LOG_INF("* RndA_enc:  ");
        LOG_HEXDUMP_INF(u8_RndA_enc, (uint16_t)s32_RandomSize, LF);
        LOG_INF("* RndA_dec:  ");
        LOG_HEXDUMP_INF(u8_RndA_dec, (uint16_t)s32_RandomSize, LF);
        LOG_INF("* RndA_rot:  ");
        LOG_HEXDUMP_INF(u8_RndA_rot, (uint16_t)s32_RandomSize, LF);
    }

    // Last step: Check if the received random A is equal to the sent random A.
    if (memcmp(u8_RndA_dec, u8_RndA_rot, s32_RandomSize) != 0)
    {
        LOG_INF("Authentication failed (3)\r\n");
        return false;
    }
...
bool Desfire::ChangeKey(uint8_t u8_KeyNo, DESFireKey* pi_NewKey, DESFireKey* pi_CurKey)
{
    ...
    {
        LOG_INF("* Cryptogram:  ");
        LOG_HEXDUMP_INF(i_Cryptogram.GetData(), (uint16_t)s32_CryptoLen, LF);
        LOG_INF("* Cryptog_enc: ");
        LOG_HEXDUMP_INF(u8_Cryptogram_enc, (uint16_t)s32_CryptoLen, LF);
    }
...
bool Desfire::GetKeyVersion(uint8_t u8_KeyNo, uint8_t* pu8_Version)
{
    ...
    if (mu8_DebugLevel > 0)
    {
        LOG_INF("Version: 0x%hhn", pu8_Version);
    }
    return true;
}
...
{
    if (mu8_DebugLevel > 0) LOG_INF("\r\n*** GetCardVersion()\r\n");

    uint8_t* pu8_Ptr = (uint8_t*)pk_Version;

    DESFireStatus e_Status;
    int32_t s32_Read = DataExchange(DF_INS_GET_VERSION, NULL, pu8_Ptr, 7, &e_Status, MAC_TmacRmac);
    ...
    if (mu8_DebugLevel > 0)
    {
        LOG_INF("--- Desfire Card Details ---\r\n");
        LOG_INF("Hardware Version: %d.%d\r\n", pk_Version->hardwareMajVersion, pk_Version->hardwareMinVersion);
        LOG_INF("Software Version: %d.%d\r\n", pk_Version->softwareMajVersion, pk_Version->softwareMinVersion);
        LOG_INF("EEPROM size:      %d uint8_t\r\n", 1 << (pk_Version->hardwareStorageSize / 2));
        LOG_INF("Production:       week %X, year 20%02X\r\n", pk_Version->cwProd, pk_Version->yearProd);
        LOG_INF("UID no:           ");         
        LOG_HEXDUMP_INF(pk_Version->uid, 7, LF);
        LOG_INF("Batch no:         ");         
        LOG_HEXDUMP_INF(pk_Version->batchNo, 5, LF);
    }
    return true;
}
...
/**************************************************************************
    ...
    In Random UID mode the card sends a 4 uint8_t UID that always starts with 0x80.
...
bool Desfire::GetRealCardID(uint8_t u8_UID[7])
{
    ...
    // The CRC must be calculated over the UID + the status uint8_t appended
...
bool Desfire::GetApplicationIDs(uint32_t u32_IDlist[28], uint8_t* pu8_AppCount)
{
    ...
    RX_BUFFER(i_RxBuf, 28*3); // 3 uint8_t per application
    uint8_t* pu8_Ptr = i_RxBuf;

    DESFireStatus e_Status;
    int32_t s32_Read1 = DataExchange(DF_INS_GET_APPLICATION_IDS, NULL, pu8_Ptr, MAX_FRAME_SIZE, &e_Status, MAC_TmacRmac);
    if (s32_Read1 < 0)
        return false;
    ...
    *pu8_AppCount = (s32_Read1 + s32_Read2) / 3;
    ...
    // Convert 3 uint8_t array -> 4 uint8_t array
    ...
    if (mu8_DebugLevel > 0)
    {
        if (*pu8_AppCount == 0)
        {
            LOG_INF("No Application ID's.\r\n");
        }
        else for (uint8_t i=0; i<*pu8_AppCount; i++)
        {
            LOG_INF("Application %2d: 0x%06X\r\n", i, (unsigned int32_t)u32_IDlist[i]);
        }
    }
    return true;
}
...
{   
    LOG_INF("\r\n*** CreateApplication(App= 0x%06X, KeyCount= %d, Type= %s)\r\n", (unsigned int32_t)u32_AppID, u8_KeyCount, DESFireKey::GetKeyTypeAsString(e_KeyType));
...
bool Desfire::GetFileIDs(uint8_t* u8_FileIDs, uint8_t* pu8_FileCount)
{
    ...
    int32_t s32_Read = DataExchange(DF_INS_GET_FILE_IDS, NULL, u8_FileIDs, 32, NULL, MAC_TmacRmac);
    if (s32_Read < 0)
        return false;
    ...
    *pu8_FileCount = s32_Read;
    ...
    if (mu8_DebugLevel > 0)
    {
        ...
        {
            LOG_INF("File ID's: ");
            LOG_HEXDUMP_INF(u8_FileIDs, (uint16_t)s32_Read, LF);
        }
    }
    return true;
}
...
bool Desfire::GetFileSettings(uint8_t u8_FileID, DESFireFileSettings* pk_Settings)
{
    ...
    int32_t s32_Read = DataExchange(DF_INS_GET_FILE_SETTINGS, &i_Params, i_RetData, 20, NULL, MAC_TmacRmac);
    if (s32_Read < 7)
        return false;
    ...
    if (mu8_DebugLevel > 0)
    {
        LOG_INF("Type: %d, Encrypt: %d, Access Read: 0x%X, Write: 0x%X, Rd+Wr: 0x%X, Change: 0x%X\r\n", 
    ...
    switch (pk_Settings->e_FileType)
    {
        ...
        case MDFT_BACKUP_DATA_FILE:
            ...
            if (mu8_DebugLevel > 0)
            {
                LOG_INF("FileSize: %d\r\n", (int32_t)pk_Settings->u32_FileSize);
            }
            return true;
        ...
        case MDFT_VALUE_FILE_WITH_BACKUP:
            ...
            if (mu8_DebugLevel > 0)
            {
                LOG_INF("LowerLimit: %d, UpperLimit: %d, CreditValue: %d, LimitEnabled: %d\r\n", 
                        (int32_t)pk_Settings->u32_LowerLimit, (int32_t)pk_Settings->u32_UpperLimit, (int32_t)pk_Settings->u32_LimitedCreditValue, (int32_t)pk_Settings->b_LimitedCreditEnabled);
            }
            return true;
        ...
        case MDFT_CYCLIC_RECORD_FILE_WITH_BACKUP:
            ...
            if (mu8_DebugLevel > 0)
            {
                LOG_INF("RecordSize: %d, MaxRecords: %d, CurrentRecords: %d\r\n", 
                        (int32_t)pk_Settings->u32_RecordSize, (int32_t)pk_Settings->u32_MaxNumberRecords, (int32_t)pk_Settings->u32_CurrentNumberRecords);
            }
            return true;
...
**************************************************************************/
bool Desfire::CreateStdDataFile(uint8_t u8_FileID, DESFireFilePermissions* pk_Permis, int32_t s32_FileSize)
{
    if (mu8_DebugLevel > 0)
    {
...
**************************************************************************/
bool Desfire::ReadFileData(uint8_t u8_FileID, int32_t s32_Offset, int32_t s32_Length, uint8_t* u8_DataBuffer)
{
    if (mu8_DebugLevel > 0)
    {
    ...
    while (s32_Length > 0)
    {
        int32_t s32_Count = MIN(s32_Length, 48); // the maximum that can be transferred in one frame (must be a multiple of 16 if encryption is used)
        ...
        DESFireStatus e_Status;
        int32_t s32_Read = DataExchange(DF_INS_READ_DATA, &i_Params, u8_DataBuffer, s32_Count, &e_Status, MAC_TmacRmac);
...
**************************************************************************/
bool Desfire::WriteFileData(uint8_t u8_FileID, int32_t s32_Offset, int32_t s32_Length, const uint8_t* u8_DataBuffer)
{
    if (mu8_DebugLevel > 0)
    {
    ...
    while (s32_Length > 0)
    {
        int32_t s32_Count = MIN(s32_Length, MAX_FRAME_SIZE - 8); // DF_INS_WRITE_DATA + u8_FileID + s32_Offset + s32_Count = 8 bytes
        ...
        DESFireStatus e_Status;
        int32_t s32_Read = DataExchange(DF_INS_WRITE_DATA, &i_Params, NULL, 0, &e_Status, MAC_TmacRmac);
        if (e_Status != ST_Success || s32_Read != 0)
            return false; // ST_MoreFrames is not allowed here!
...
/**************************************************************************
   ...
   s32_RecvSize   = buffer size of u8_RecvBuf
    pe_Status     = if (!= NULL) -> receives the status uint8_t
    e_Mac         = defines CMAC calculation
    returns the uint8_t count that has been read into u8_RecvBuf or -1 on error
**************************************************************************/
int32_t Desfire::DataExchange(uint8_t u8_Command, TxBuffer* pi_Params, uint8_t* u8_RecvBuf, int32_t s32_RecvSize, DESFireStatus* pe_Status, DESFireCmac e_Mac)
{
...
int32_t Desfire::DataExchange(TxBuffer* pi_Command,               // in (command + params that are not encrypted)
                          TxBuffer* pi_Params,                // in (parameters that may be encrypted)
                          uint8_t* u8_RecvBuf, int32_t s32_RecvSize, // out
                          DESFireStatus* pe_Status,           // out
                          DESFireCmac    e_Mac)               // in
{
    if (pe_Status) *pe_Status = ST_Success;
    mu8_LastPN532Error = 0;
    uint8_t skip = 1; // skip the first byte (status uint8_t)

    TX_BUFFER(i_Empty, 1);
    if (pi_Params == NULL)
        pi_Params = &i_Empty;

    // - Status uint8_t from Desfire card (0 if no error)
    // - data bytes ...
    int32_t s32_Overhead = 1; // Overhead added to payload = card status uint8_t
    if (e_Mac & MAC_Rmac) s32_Overhead += 8; // + 8 bytes for CMAC
  
    // mu8_PacketBuffer is used for input and output
    if (2 + pi_Command->GetCount() + pi_Params->GetCount() > CLRC663_PACKBUFFSIZE || s32_Overhead + s32_RecvSize > CLRC663_PACKBUFFSIZE)    
    {
        LOG_ERR("DataExchange(): Invalid parameters");
        return -1;
    }
    ...
    if (e_Mac & MAC_Tcrypt) // CRC and encrypt pi_Params
    {
        ...
        int32_t s32_CryptCount = mpi_SessionKey->CalcPaddedBlockSize(pi_Params->GetCount());
    ...
    { 
        ...
        {
            LOG_INF("TX CMAC:  ");
            LOG_HEXDUMP_INF(u8_CalcMac, (uint16_t)(mpi_SessionKey->GetBlockSize()), LF);
        }
    }

    int8_t P=0;

    bytecpy(mu8_PacketBuffer + P, pi_Command->GetData(), pi_Command->GetCount());
    P += pi_Command->GetCount();

    bytecpy(mu8_PacketBuffer + P, pi_Params->GetData(),  pi_Params->GetCount());
    P += pi_Params->GetCount();
    
    LOG_HEXDUMP_ERR(mu8_PacketBuffer, uint32_t(P), "RAW data send: ");
    int16_t s32_Len = nfc_card_iso14443_4_com(clrc663, mu8_PacketBuffer, P, mu8_PacketBuffer, 255);
    if (s32_Len < 0) {
        LOG_ERR("DataExchange() failed %d", s32_Len);
        return -1;
    } else if(s32_Len == 0) {
        return 0;
    }
    LOG_HEXDUMP_DBG(mu8_PacketBuffer, (uint32_t)s32_Len, "RAW data receive: ");

    // ReadData() returns 1 uint8_t if status error from the Desfire card
    if (s32_Len < 1)
    {
        LOG_ERR("DataExchange() failed");
        return -1;
    }
    
    uint8_t u8_CardStatus  = mu8_PacketBuffer[0]; // contains errors from the Desfire card
    ...
    s32_Len -= skip; // 1 byte uint8_t card status
        ...
        (mu8_LastAuthKeyNo != NOT_AUTHENTICATED))                          // No session key -> no CMAC calculation possible
    {
        ...
        {
            if (!mi_CmacBuffer.AppendBuf(mu8_PacketBuffer + skip, s32_Len))
                return -1;
        }
           ...
           (u8_CardStatus == ST_Success)) // Response contains CMAC only in case of success
        {
            ...
            uint8_t* u8_RxMac = mu8_PacketBuffer + skip + s32_Len;
            
            // The CMAC is calculated over the RX data + the status uint8_t appended to the END of the RX data!
            if (!mi_CmacBuffer.AppendBuf(mu8_PacketBuffer + skip, s32_Len) ||
            ...
            // For AES the CMAC is 16 uint8_t, but only 8 are transmitted
            if (memcmp(u8_RxMac, u8_CalcMac, 8) != 0)
            {
                LOG_INF("CMAC Mismatch\r\n");
                return -1;
            }
        }
    }
    ...
    {
        LOG_INF("DataExchange() Buffer overflow\r\n");
        return -1;
    } 

    if (u8_RecvBuf && s32_Len)
    {
        memcpy(u8_RecvBuf, mu8_PacketBuffer + skip, s32_Len);
        ...
        if (e_Mac & MAC_Rcrypt) // decrypt received data with session key
        {
            ...
            {
                LOG_INF("Decrypt:  ");
                LOG_HEXDUMP_INF(u8_RecvBuf, (uint16_t)s32_Len, LF);
            }        
        }    
    }
    return s32_Len;
...
// Checks the status uint8_t that is returned from the card
bool Desfire::CheckCardStatus(DESFireStatus e_Status)
{
    ...
    LOG_ERR("Desfire Error: ");
    switch (e_Status)
    {
        case ST_OutOfMemory:
            LOG_ERR("Not enough EEPROM memory.\r\n");
            return false;
        case ST_IllegalCommand:
            LOG_ERR("Illegal command.\r\n");
            return false;
        case ST_IntegrityError:
            LOG_ERR("Integrity error.\r\n");
            return false;
        case ST_KeyDoesNotExist:
            LOG_ERR("Key does not exist.\r\n");
            return false;
        case ST_WrongCommandLen:
            LOG_ERR("Wrong command length.\r\n");
            return false;
        ...
        case ST_IncorrectParam:
            ...
            return false;
        case ST_AppNotFound:
            LOG_ERR("Application not found.\r\n");
            return false;
        case ST_AppIntegrityError:
            LOG_ERR("Application integrity error.\r\n");
            return false;
        case ST_AuthentError:
            LOG_ERR("Authentication error.\r\n");
            return false;
        case ST_LimitExceeded:
            LOG_ERR("Limit exceeded.\r\n");
            return false;
        case ST_CardIntegrityError:
            LOG_ERR("Card integrity error.\r\n");
            return false;
        case ST_CommandAborted:
            LOG_ERR("Command aborted.\r\n");
            return false;
        case ST_CardDisabled:
            LOG_ERR("Card disabled.\r\n");
            return false;
        case ST_InvalidApp:
            LOG_ERR("Invalid application.\r\n");
            return false;
        case ST_DuplicateAidFiles:
            LOG_ERR("Duplicate AIDs or files.\r\n");
            return false;
        case ST_EepromError:
            LOG_ERR("EEPROM error.\r\n");
            return false;
        case ST_FileNotFound:
            LOG_ERR("File not found.\r\n");
            return false;
        case ST_FileIntegrityError:
            LOG_ERR("File integrity error.\r\n");
            return false;
        default:
            LOG_ERR("0x");
...
bool Desfire::Selftest()
{
    ...
    u8_Length = nfc_card_detect_t(clrc663, u8_UID, &u8_SAK, u8_ATQA, u8_ATS);
    if (u8_Length != 0)
    {
        LOG_INF("No card found.\r\n");
        return false;
    } 

    if (u8_ATS[0] == 0)
    {
        LOG_INF("The selftest requires a Desfire card.\r\n");
        return false;
    }
    ...
    if (u8_Version != 0)
    {
        LOG_INF("The selftest requires an empty Desfire card (factory default DES key)\r\n");
        return false;
    }
    ...
    {
        LOG_INF("GetApplicationIDs() failed\r\n");
        return false;
    }
    ...
    if (u8_AppCount != 3)
    {
        LOG_INF("DeleteApplication() failed\r\n");
        return false;
    }
    ...
    const int32_t FILE_LENGTH = 80; // this exceeds the frame size -> requires two frames for write / read    
    ...
    {
        LOG_INF("GetFileSettings() failed\r\n");
        return false;
    }
    ...
    {
        LOG_INF("Read/Write file failed\r\n");
        return false;
    }
...
{ 
    ...
    {
        LOG_INF("ChangeKeySettings() failed\r\n");
        return false;
    }                
