...
public:
    ...
    virtual bool SetKeyData(const uint8_t* u8_Key, int32_t s32_KeySize, uint8_t u8_Version) = 0;
    ...
    bool CryptDataCBC(DESFireCBC e_CBC, DESFireCipher e_Cipher, uint8_t* u8_Out, const uint8_t* u8_In, int32_t s32_ByteCount)
    {
        ...
        uint8_t u8_Temp[16];
        for (int32_t B=0; B<s32_ByteCount/ms32_BlockSize; B++)
        {
            if (e_CBC == CBC_SEND)
            {
    ...
    // In case of a simple DES key (8 uint8_t long) the upper 8 uint8_t and the lower 8 uint8_t are identical. (See DES::SetKeyData())
    // The Desfire card detects automatically that this 16 uint8_t key is really a simple 8 uint8_t DES key.
    inline int32_t GetKeySize(int32_t s32_MinSize=0) 
    { 
    ...
    int32_t CalcPaddedBlockSize(int32_t s32_ByteCount)
    {
    ...
    {
        // Even if an 8 uint8_t simple DES key is used, the function ChangeKey() works internally always with 16 bytes.
        // For a simple DES key 16 uint8_t will be printed where the first half is idententical to the second half.
    ...
    static const char* GetKeyTypeAsString(DESFireKeyType e_KeyType, int32_t s32_KeySize=0)
    {
        switch (e_KeyType)
        {
            case DF_KEY_2K3DES: 
...
protected:
    ...
    uint8_t mu8_Key[24];
    int32_t  ms32_KeySize;
    int32_t  ms32_BlockSize;
    uint8_t mu8_Version;
