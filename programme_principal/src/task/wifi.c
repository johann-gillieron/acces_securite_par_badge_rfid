/*! @file wifi.c
 * @brief This file contains the implementation of the Wifi task.
 *
 * @author Johann Gilliéron
 * @date 2024-07-18
 * 
 * @note This file is a modified verion of the TWT sample from Nordic Semiconductor
 * 		 With the following modifications:
 * 			- The main function is removed
 * 			- Now it is a thread that initializes the wifi connection
 * 			- The wifi is initialized in the wifi_init_thread function
 * 			- The wifi thread is started in the Communication_thread_init function
 */
...
static int teardown_twt(void)
{
	...
	wifi_context.twt_enabled = false;
...
// This is the function used to actualaiez the wifi state
				     ...
				     uint32_t mgmt_event, struct net_if *iface)
{
	...
	case NET_EVENT_WIFI_TWT_SLEEP_STATE: {
		...
		if (*twt_state == WIFI_TWT_STATE_SLEEP) {
			//tcp_traffic_pause(&comm_context);
			comm_context.pause_traffic = true;
		} else if (*twt_state == WIFI_TWT_STATE_AWAKE)  {
			//tcp_traffic_resume(&comm_context);
			comm_context.pause_traffic = false;
		} else {
...
/*! wifi_init_thread initializes the wifi thread
 *
 *@brief wifi_init_thread make the complete connection process.
 */
...
wifi_begin:
		...
		while (!wifi_context.connect_result) 
		{
			cmd_wifi_status();
			k_sleep(K_MSEC(STATUS_POLLING_MS));
		}
...
server_connect:
		if (wifi_context.connected) {
			...
			k_sleep(K_SECONDS(3)); // Wait the wifi connection to be stable
			// Before starting the TWT tests, initialize the TCP client
			if (!tcp_client_init(&comm_context)) 
			{
				LOG_ERR("Failed to initialize TCP client\n");
				goto server_connect;
			}

			if (!twt_supported) 
			{
				LOG_INF("AP is not TWT capable, exiting\n");
			} 
			else 
			{
...
begin_communication:
			ret = communication_loop(&comm_context);
			if (ret < 0) {
				LOG_ERR("Communication loop failed: %d", ret);
				// Reboot the device
				LOG_ERR("System Error : Reboot\n");
				k_sleep(K_SECONDS(2));
				sys_reboot(SYS_REBOOT_WARM);
			}
			if (ret == 1) {
				LOG_INF("Server disconnected, reconnecting\n");
				close(comm_context.tcp_sock_fd);
				comm_context.tcp_sock_fd = -1;
...
// TODO: Implement the update process
		...
		{
			LOG_INF("Connection failed, retrying\n");
			goto wifi_begin;
		}
	}
	k_sleep(K_FOREVER);
...
/*! wifi_init_thread initializes the wifi thread
 *
 *@brief Initialize the wifi thread
 */
void communication_thread_init(void) {
	k_thread_create(&wifiThread, WIFI_STACK, K_THREAD_STACK_SIZEOF(WIFI_STACK),
			(k_thread_entry_t)communication_thread, NULL, NULL, NULL,
			WIFI_PRIORITY, 0, K_NO_WAIT);
	k_thread_name_set(&wifiThread, "Communication_thread");
